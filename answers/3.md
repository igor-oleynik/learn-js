...
function deleteFruit(fruitTwo, fruitElement) {
    fruitElement = 'Apple';
    fruitTwo = fruit.filter(item => item !== fruitElement);
...
const studentsObjact = students.forEach(item => console.log(item)) ;
```
```js
// нужно переделать, убрать пустой объект
const students = [
  { id: 1, name: 'Quincy', grade: 21 },
  { id: 2, name: 'Jason', grade: 33 },
  { id: 3, name: 'Alexis', grade: 87 },
...
const studentsObjact = {};
students.forEach(array => {
    studentsObjact[array.id] = array;
});
...
const fruit = ['Apple', 'Orange', 'Plum', 'Pineapple'];
const [...fruits] = fruit;
const lastFruitArray = fruits[fruits.length -1]

console.log(lastFruitArray);
```
```js
// 2
const fruit = ['Apple', 'Orange', 'Plum', 'Pineapple'];
const [...fruits] = fruit;
const lastFruitArray = fruits.slice(-1)[0];
